...
class SpaFHy():
    ...
    def run_timestep(self, forc, ncf=False, flx=False):
        ...
        if ncf:
            ...
            ss = self.top.local_s(self.top.S) # compute local saturation deficit from catchment average
            ss[ss < 0] = 0.0 # set sat. deficit at saturated cells (s <0) equal to 0
            ncf['top']['Sloc'][k,:,:] = self._to_grid(ss) # convert to grid format
            del ss
        # update step number
        self.step_nr += 1
